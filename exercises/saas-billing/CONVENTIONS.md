...
The pinned choices every query in this directory implements. The warehouse
README (`../../datasets/saas-billing/README.md`, § Billing spec) states the
stage order and the deliberate ambiguities; this file records which side of
each ambiguity the **reference** takes, so that `scripts/check.sh` against the
reference is an exact-equality test. A candidate query that picks a different
convention is not wrong about billing — it is wrong about *this* spec, and the
diff will say where.
